...
import threading
# Set up serial connection
serial_port = 'COM6'  # Adjust this to your system's port
baud_rate = 115200
ser = serial.Serial(serial_port, baud_rate)
...
curves = []
num_channels = 18
serial_buffer_size = num_channels * 2 
for i in range(num_channels):
    curve = plot.plot(pen=(i, num_channels))
    curves.append(curve)
...
def decode_serial_data(data):
    try:
        readings = struct.unpack('<' + 'H' * num_channels, data)
        return readings
    ...
    if ser.in_waiting:
        try:
            while(ser.in_waiting):
                adc_data=read_serial()
                voltage=(adc_data/1024)*3.3
                for i in range(num_channels):
                    # Shift buffer and insert new voltage value
                    data_buffers[i][:-1] = data_buffers[i][1:]
                    data_buffers[i][-1] = voltage[i]  # Store voltage instead of ADC value
                    if checkboxes[i].isChecked():
                            curves[i].setData(x, data_buffers[i])
...
timer = QtCore.QTimer()
timer.timeout.connect(update)
timer.start(100)  # Update every 50ms
